...
TRAINING_SCRIPT = REPO_ROOT / "src" / "detectors" / "yolo" / "train_ultralytics.py"
...
async def run_training_job(job_id: str, config: TrainingConfig):
    ...
    try:
        ...
        dataset_path = await loop.run_in_executor(
            None, download_roboflow_dataset_sync, config.roboflow_dataset, dataset_dir
        )
        ...
        training_jobs[job_id]["logs"] = (
            log_lines[-500:] if len(log_lines) > 500 else log_lines
        )
                ...
                gpus.append(
                    {
                        ...
                        "free_memory_gb": round(
                            (props.total_memory - memory_reserved) / (1024**3), 2
                        ),
                        "utilization": round(
                            (memory_reserved / props.total_memory) * 100, 1
                        ),
                    }
                )
